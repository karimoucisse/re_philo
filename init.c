...
int	init_prog(char **av, t_prog *prog, t_philo *philo, pthread_mutex_t *forks)
{
...
int	init_struct(char **av, t_prog *prog, t_philo *philo, pthread_mutex_t *forks)
{
	int	i;
	...
	if (!init_prog(av, prog, philo, forks))
		return (0);
	if (!init_philo(av, philo, prog, forks))
		return (0);
	return (1);
}
